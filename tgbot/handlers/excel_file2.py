...
async def in_progress(query: CallbackQuery):
    try:
        ...
        if not excel_data.empty:
            ...
            await query.answer()
            await query.message.answer(f"<b>{today_date} gacha quyidagi {x} ta obektlarda montaj ishlari yakunlangan, lekin ishga tushmagan:</b>\n"
                                 f"{ne_zapushen}\n",
                                 reply_markup=None)
        else:
            await query.answer()
    ...
    try:
        if is_user(query.from_user.id):
            ...
            def create_percentage_chart():
                ...
                ax.set_title('Viloyatlar kesimida ishga tushgan BS larning foiz ulushi', fontsize=16)
            ...
            await query.message.answer_photo(
                ...
                caption=f"{today_date} sanasida viloyatlar kesimida ishga tushgan tayanch stansiyalari foizi"
            )
            ...
            def create_quantity_chart():
                ...
                ax.set_title("Viloyatlar kesimida ishga tushgan va modernizatsiya bo'lgan BS lar", fontsize=16)
                ax.set_xlabel('Viloyatlar', fontsize=12)
            ...
            await query.message.answer_photo(
                ...
                caption=f"{today_date} sanasida viloyatlar kesimida ishga tushgan tayanch stansiyalari soni"
            )

            # Generate and send detailed statistics as text
            stats_text = f"{today_date} sanasida viloyatlar kesimida ishga tushgan BS lar xaqida umumiy ma'lumot:\n\n"
            for region in total_stations.index:
                ...
                stats_text += f"{region}:\n"
                stats_text += f"  Total BS: {total}\n"
                stats_text += f"  Launched BS: {launched}\n"
                stats_text += f"  Percentage Launched: {percentage:.2f}%\n"
